...
def get_current_info():
    ...
    current_is_day = data["current"].get("is_day", None) #Goes through the data and looks for current/is_day value in dictionary thing
    current_time = data["current"].get("time", None) #
    current_percipitation_status = data["current"].get("time", None)
    current_overall_status = data["current"].get("weather_code", None)




    return render_template("index.html", isDay=current_is_day, currentTime=current_time, currentPercipitationStatus=current_percipitation_status, currentOverallStatus=current_overall_status) #Think global var, exports data to the html page
